// TODO: upstream this into CRDsonnet
...

{
  ...
  nestInParents(name, parents, field)::
    local startParents =
      if autils.isObject(field)
      then parents[:std.length(parents) - 1]
      else parents;
    local startField =
      if autils.isField(field) || autils.isFieldFunction(field)
      then field
      else if autils.isObject(field)
      then
        a.field.new(
      ...
      else error '`field` is of type %s, must be a field type' % autils.type(field);
    [
      std.foldr(
        function(p, acc)
          if p == name
